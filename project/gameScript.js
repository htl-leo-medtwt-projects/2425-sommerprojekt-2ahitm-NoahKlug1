...
const spellIndicator = document.getElementById("spellIndicator");
...
// ---------------------------
// TROOP-TYPEN & DECK
// Neben den bekannten Einheiten gibt es zusätzlich den Spell "blitz".
const cardPool = [
...
// ---------------------------
// SPELL: Blitz
function castBlitz(x, y) {
  const radius = 80;
  let start = performance.now();
  function animateBlitz(timestamp) {
    ...
    if (elapsed < 500) {
      ...
      ctx.lineWidth = 3;
      ctx.stroke();
      requestAnimationFrame(animateBlitz);
    } else {
      [playerUnits, enemyUnits].forEach(units => {
        units.forEach(unit => {
          const d = getDistance({x, y, width: 0, height: 0}, unit);
          if (d <= radius) {
            unit.hp -= 50;
            unit.flashTimer = 0.2;
          }
...
requestAnimationFrame(gameLoop);
// Spell-Indikator: Zeigt Blitz-Radius an, wenn Blitz ausgewählt ist.
canvas.addEventListener("mousemove", (event) => {
  if (selectedCard !== null && playerHand[selectedCard] === "blitz") {
    const rect = canvas.getBoundingClientRect();
    const mouseX = event.clientX - rect.left;
    const mouseY = event.clientY - rect.top;
    spellIndicator.style.display = "block";
    const radius = 80;
    spellIndicator.style.width = radius*2 + "px";
    spellIndicator.style.height = radius*2 + "px";
    spellIndicator.style.left = (mouseX - radius) + "px";
    spellIndicator.style.top = (mouseY - radius) + "px";
  } else {
    spellIndicator.style.display = "none";
  }
});
...
*/
// ---------------------------
/*/ Helfer: Abstand zwischen Mittelpunkten
function getDistance(a, b) {
  const ax = a.x + a.width / 2, ay = a.y + a.height / 2;
  const bx = b.x + b.width / 2, by = b.y + b.height / 2;
  return Math.sqrt((ax - bx) ** 2 + (ay - by) ** 2);
...
*/
// ---------------------------
/*/ Verhindert, dass Einheiten durch lebende Türme laufen.
function collidesWithTowers(unit, towers) {
  for (let tower of towers) {
    if (tower.hp > 0) {
      if (unit.x < tower.x + tower.width &&
...
*/
// ---------------------------
/*/ Bewegung & Angriff: Die Einheiten bleiben am Ziel stehen, greifen aber in einem Bereich an.
function moveUnit(unit, deltaTime) {
  let targetPos;
  if (unit.type === "giant") {
    let candidate = null;
    ...
    towers.forEach(tower => {
      if (tower.type === "small" && tower.hp > 0) {
        let d = getDistance(unit, tower);
        if (d < candidateDist) { candidate = tower; candidateDist = d; }
      }
    });
    if (candidate) {
      targetPos = { x: candidate.x + candidate.width / 2, y: candidate.y + candidate.height / 2 };
  ...
  if (unit.owner === "player") {
    if (unit.x + unit.width < riverX && targetPos.x > riverX) {
      let bridgeY = Math.abs((unit.y + unit.height / 2) - bridgeTop.centerY) < Math.abs((unit.y + unit.height / 2) - bridgeBottom.centerY)
                    ? bridgeTop.centerY : bridgeBottom.centerY;
      targetPos = { x: riverX + riverWidth / 2, y: bridgeY };
    }
  } else {
    if (unit.x > riverX + riverWidth && targetPos.x < riverX + riverWidth) {
      let bridgeY = Math.abs((unit.y + unit.height / 2) - bridgeTop.centerY) < Math.abs((unit.y + unit.height / 2) - bridgeBottom.centerY)
                    ? bridgeTop.centerY : bridgeBottom.centerY;
      targetPos = { x: riverX + riverWidth / 2, y: bridgeY };
    }
  ...
  let vx = dx / dist, vy = dy / dist;
  let moveDist = unit.speed * deltaTime;
  let desiredX = unit.x + vx * moveDist;
  let desiredY = unit.y + vy * moveDist;
  desiredX = Math.max(0, Math.min(desiredX, canvas.width - unit.width));
  desiredY = Math.max(0, Math.min(desiredY, canvas.height - unit.height));
  let adjusted = avoidTowerCollision(unit, desiredX, desiredY);
  desiredX = adjusted.x;
  desiredY = adjusted.y;
  const friendly = unit.owner === "player" ? playerUnits : enemyUnits;
...
*/
// ---------------------------
/*/ Towers verhindern das Durchlaufen: Versucht, Einheiten aus der Kollision zu schieben.
function collidesWithTowers(unit, towers) {
  for (let tower of towers) {
    if (tower.hp > 0) {
      if (unit.x < tower.x + tower.width &&
...
function updateUnits(deltaTime) {
  ...
  updateFlashTimers(playerUnits, deltaTime);
  updateFlashTimers(enemyUnits, deltaTime);
}
...
function draw() {
  ...
  function drawUnit(unit) {
    ctx.fillStyle = unit.flashTimer > 0 ? "red" : unit.color;
